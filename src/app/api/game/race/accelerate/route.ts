...
export async function POST(req: NextRequest) {
  try {
    ...
    const { gameId, boostPercent } = await req.json();
    if (!gameId || !boostPercent) return safeJsonResponse({ error: 'Paramètres requis' }, { status: 400 });
    if (boostPercent < 1 || boostPercent > 100) return safeJsonResponse({ error: 'boostPercent invalide' }, { status: 400 });
    ...
    const vmax = carStats.max_speed ? carStats.max_speed / 3.6 : 0;
    const masse = carStats.weight || 1200;
    const cv = carStats.power || 120;
    const alpha = boostPercent / 100;
    const t_0_100 = carStats.zero_hundred || 8.0;
    const cd = carStats.drag_coefficient || 0.3;
    const area = carStats.frontal_area || 2.0;
    const mu = carStats.rrc || 0.015;
    const { vitesse, distance } = calculerVitesseEtDistanceApres5s(v0, vmax, masse, cv, alpha, t_0_100, cd, area, mu);
    ...
    for (const turn of map.turns) {
      for (const interval of turn.m) {
        ...
        // On vérifie sur chaque tour franchi
